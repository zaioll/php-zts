...
DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends $(cat /install/requirements/*packages | tr "\n" " ")
...
ln -s /usr/lib/x86_64-linux-gnu/libsodium.so.23 /usr/lib/libsodium.so.23

echo "Ajustando timezone, data e hora"
export TZ=${TZ-:"America/Manaus"}
